// positive-inside-f.zig
/// Return true if incoming Float array has at least one positive element
/// Arguments:
///     a - incoming array
pub fn positive_inside_F(a: []f32) bool {
    const Vec = blk: {
    ...
    while (i < simd_len) : (i += 1) {
        ...
        const cmp = vec > @as(Vec, @splat(0.0));
        if (@reduce(.Or, cmp)) {
            return true;
        }
    }
