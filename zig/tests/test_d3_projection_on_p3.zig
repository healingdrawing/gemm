...
const data_d3_projection_on_p3 = @import("data_d3_projection_on_p3.zig");
const floatUtils = @import("float.zig");
const gemm = @import("../gemm.zig").GEMM;
const report = @import("report.zig");

pub fn test_d3_projection_on_p3(epsilon: f32) !report.MethodResult {
    const allocator = std.heap.page_allocator;
    var failed: usize = 0;

    dp.devlog(.{"Running d3_projection_on_p3 tests..."});

    for (data_d3_projection_on_p3.cases) |case| {
        // Call Zig method
        const zig_result = try floatUtils.to_array(allocator, gemm.d3_projection_on_p3(case.d3, case.p3));
        ...
        const cmd = try std.fmt.allocPrint(allocator, "/home/user/.bun/bin/bun ../ts/terminalcall.ts d3_projection_on_p3 {s}", .{data_str});
        ...
        if (ok_zig_ts and ok_zig_exp and ok_ts_exp) {
            std.debug.print("✓ d3_projection_on_p3({any}, {any}):\nZig={any},\n TS={any}\n\n", .{ case.d3, case.p3, zig_result, ts_result });
        } else {
            failed += 1;
            dp.errlog(.{
                "✗ d3_projection_on_p3",
    ...
    return .{
        .name = "d3_projection_on_p3",
        .total = data_d3_projection_on_p3.cases.len,
        .failed = failed,
    };
}
