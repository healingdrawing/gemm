// tests/test_v3v3paralleled_sameside.zig
const std = @import("std");
const dp = @import("../utils/debug.zig");
const data_v3v3paralleled_sameside = @import("data_v3v3paralleled_sameside.zig");
const floatUtils = @import("float.zig");
const gemm = @import("../gemm.zig").GEMM;
const report = @import("report.zig");

pub fn test_v3v3paralleled_sameside(epsilon: f32) !report.MethodResult {
    _ = epsilon; // not used for exact bool threshold, keep signature identical
    const allocator = std.heap.page_allocator;
    var failed: usize = 0;

    dp.devlog(.{"Running v3v3paralleled_sameside tests..."});

    for (data_v3v3paralleled_sameside.cases) |case| {
        // Call Zig method (returns bool)
        const zig_bool = gemm.v3v3paralleled_sameside(case.a, case.b);
        ...
        const cmd = try std.fmt.allocPrint(allocator, "/home/user/.bun/bin/bun ../ts/terminalcall.ts v3v3paralleled_sameside {s}", .{data_str});
        ...
        if (ok_zig_ts and ok_zig_exp and ok_ts_exp) {
            std.debug.print("✓ v3v3paralleled_sameside({any}, {any}) = {any}\n", .{ case.a, case.b, zig_result });
        } else {
            failed += 1;
            dp.errlog(.{
                "✗ v3v3paralleled_sameside",
                "a",
                case.a,
                "b",
                case.b,
    ...
    return .{
        .name = "v3v3paralleled_sameside",
        .total = data_v3v3paralleled_sameside.cases.len,
        .failed = failed,
    };
}
