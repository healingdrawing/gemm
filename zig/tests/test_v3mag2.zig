...
pub fn test_v3mag2(epsilon: f32) !void {
    ...
    for (data_v3mag2.cases) |case| {
        // Call Zig method
        const zig_result = try floatUtils.to_array(allocator, gemm.v3mag2(case.v));
        ...
        const cmd = try std.fmt.allocPrint(allocator, "/home/user/.bun/bin/bun ../ts/terminalcall.ts v3mag2 {s}", .{data_str});
