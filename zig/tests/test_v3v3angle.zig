// tests/test_v3v3angle.zig
const std = @import("std");
const dp = @import("../utils/debug.zig");
const data_v3v3angle = @import("data_v3v3angle.zig");
const floatUtils = @import("float.zig");
const gemm = @import("../gemm.zig").GEMM;
const report = @import("report.zig");

pub fn test_v3v3angle(epsilon: f32) !report.MethodResult {
    const allocator = std.heap.page_allocator;
    var failed: usize = 0;

    dp.devlog(.{"Running v3v3angle tests..."});

    for (data_v3v3angle.cases) |case| {
        // Call Zig method
        const zig_result = try floatUtils.to_array(allocator, gemm.v3v3angle(case.a, case.b));
        ...
        const cmd = try std.fmt.allocPrint(allocator, "/home/user/.bun/bin/bun ../ts/terminalcall.ts v3v3angle {s}", .{data_str});
        ...
        if (ok_zig_ts and ok_zig_exp and ok_ts_exp) {
            std.debug.print("✓ v3v3angle({any}, {any}) = {any}\n", .{ case.a, case.b, zig_result });
        } else {
            failed += 1;
            dp.errlog(.{
                "✗ v3v3angle",
    ...
    return .{
        .name = "v3v3angle",
        .total = data_v3v3angle.cases.len,
        .failed = failed,
    };
}
