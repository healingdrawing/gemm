...
const data_v3back = @import("data_v3back.zig");
const floatUtils = @import("float.zig");
const gemm = @import("../gemm.zig").GEMM;
const report = @import("report.zig");

pub fn test_v3back(epsilon: f32) !report.MethodResult {
    const allocator = std.heap.page_allocator;
    var failed: usize = 0;

    dp.devlog(.{"Running v3back tests..."});

    for (data_v3back.cases) |case| {
        // Call Zig method
        const zig_result = try floatUtils.to_array(allocator, gemm.v3back(case.v));
        ...
        const cmd = try std.fmt.allocPrint(allocator, "/home/user/.bun/bin/bun ../ts/terminalcall.ts v3back {s}", .{data_str});
        ...
        if (ok_zig_ts and ok_zig_exp and ok_ts_exp) {
            std.debug.print("✓ v3back({any}) = {any}\n", .{ case.v, zig_result });
        } else {
            failed += 1;
            dp.errlog(.{
                "✗ v3back",
