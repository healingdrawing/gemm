// tests/test_p3_d3v3v3.zig
const std = @import("std");
const dp = @import("../utils/debug.zig");
const data_p3_d3v3v3 = @import("data_p3_d3v3v3.zig");
const floatUtils = @import("float.zig");
const gemm = @import("../gemm.zig").GEMM;
const report = @import("report.zig");

pub fn test_p3_d3v3v3(epsilon: f32) !report.MethodResult {
    const allocator = std.heap.page_allocator;
    var failed: usize = 0;

    dp.devlog(.{"Running p3_d3v3v3 tests..."});

    for (data_p3_d3v3v3.cases) |case| {
        // Call Zig method
        const zig_result = try floatUtils.to_array(allocator, gemm.p3_d3v3v3(case.d3, case.v3a, case.v3b));
        defer allocator.free(zig_result);

        // Call TS bridge
        const data_str = try floatUtils.vectors_to_string(allocator, .{ case.d3, case.v3a, case.v3b });
        ...
        const cmd = try std.fmt.allocPrint(allocator, "/home/user/.bun/bin/bun ../ts/terminalcall.ts p3_d3v3v3 {s}", .{data_str});
        ...
        if (ok_zig_ts and ok_zig_exp and ok_ts_exp) {
            std.debug.print("✓ p3_d3v3v3({any}, {any}, {any}) = {any}\n", .{ case.d3, case.v3a, case.v3b, zig_result });
        } else {
            failed += 1;
            dp.errlog(.{
                "✗ p3_d3v3v3",
    ...
    return .{
        .name = "p3_d3v3v3",
        .total = data_p3_d3v3v3.cases.len,
        .failed = failed,
    };
}
