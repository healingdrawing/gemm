// src/v3rot/bench_v3rot.zig
const std = @import("std");
const v3rot = @import("v3rot.zig");
...
pub fn main(init: std.process.Init) !void {
    ...
    std.debug.print("v3rot - {} iterations\n\n", .{iterations});
    ...
    // 2. cross_temps
    {
        ...
        for (0..iterations) |_| {
            ...
            naxis = .{ rand.float(f32) * 2 - 1, rand.float(f32) * 2 - 1, rand.float(f32) * 2 - 1 };
            angle = rand.float(f32) * std.math.pi * 2 - std.math.pi;
            result = v3rot.v3rot_cross_temps(v, naxis, angle);
            sum += result[0] + result[1] + result[2];
        }
        volatile_sum += sum;
        const elapsed = start.durationTo(std.Io.Clock.real.now(init.io)).toNanoseconds();
        std.debug.print("{d:>12} ns  ({d:.3} ns/op) v3rot_cross_temps\n", .{ elapsed, @as(f64, @floatFromInt(elapsed)) / iterations });
    }
    ...
    // 4. fma dott
    {
        ...
        for (0..iterations) |_| {
            ...
            naxis = .{ rand.float(f32) * 2 - 1, rand.float(f32) * 2 - 1, rand.float(f32) * 2 - 1 };
            angle = rand.float(f32) * std.math.pi * 2 - std.math.pi;
            result = v3rot.v3rot_fma_dott(v, naxis, angle);
            sum += result[0] + result[1] + result[2];
        }
        volatile_sum += sum;
        const elapsed = start.durationTo(std.Io.Clock.real.now(init.io)).toNanoseconds();
        std.debug.print("{d:>12} ns  ({d:.3} ns/op) v3rot_fma_dott\n", .{ elapsed, @as(f64, @floatFromInt(elapsed)) / iterations });
    }
    ...
    // 3. fma
    {
        ...
        for (0..iterations) |_| {
            ...
            naxis = .{ rand.float(f32) * 2 - 1, rand.float(f32) * 2 - 1, rand.float(f32) * 2 - 1 };
            angle = rand.float(f32) * std.math.pi * 2 - std.math.pi;
            result = v3rot.v3rot_fma(v, naxis, angle);
            sum += result[0] + result[1] + result[2];
        }
        volatile_sum += sum;
        const elapsed = start.durationTo(std.Io.Clock.real.now(init.io)).toNanoseconds();
        std.debug.print("{d:>12} ns  ({d:.3} ns/op) v3rot_fma\n", .{ elapsed, @as(f64, @floatFromInt(elapsed)) / iterations });
    }
